...
int caf_plotter(std::string file_list, bool is_flat = true)
{
    ...
    for(const auto& f : root_list)
    {
        ...
        for (unsigned long i = 0; i < nspills; ++i)
        {
            ...
            for(unsigned long ixn = 0; ixn < num_ixn; ++ixn)
            {
                ...
                if((reco_nproton == 1) & (reco_nmuon == 1) & (reco_npion == 0)) // Check if reco passes
                    reco_passes = true;
                ...
                // Require mode of interaction to be QE for truth (mode = 1)
                auto truth_mode = truth_ixn.mode;

                if((truth_nproton == 1) & (truth_nmuon == 1) & (truth_npion == 0) & (truth_mode == 1)) // Check if truth passes
                    truth_passes = true;

                // If interaction is not QE1p1mu0pi (reco or truth), go to next interaction
    ...
    std::string file_name = "qe_purity_eff_output_1.0";
