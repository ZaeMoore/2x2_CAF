    ...
    for(const auto& f : root_list)
    {
        ...
        for (unsigned long i = 0; i < nspills; ++i)
        {
            ...
            for(unsigned long ixn = 0; ixn < num_ixn; ++ixn)
            {
                ...
                // Loop over particles in matched truth interaction
                for(unsigned long part = 0; part < sr->mc.nu[truth_idx].prim.size(); ++part)
                {
                    const auto& true_part = sr->mc.nu[truth_idx].prim[part];
                ...
                if(truth_nproton < 2 or truth_npion > 0 or truth_nmuon < 1)
                    continue;
